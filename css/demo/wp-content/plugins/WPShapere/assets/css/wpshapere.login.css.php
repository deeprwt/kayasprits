...
body.login{background-color:<?php echo $this->aof_options['login_bg_color'] . ' !important;'; if(!empty($login_bg_img)) echo ' background-image: url(' . $login_background  . ');'; if($this->aof_options['login_bg_img_repeat'] === true) echo 'background-repeat: repeat'; else echo 'background-repeat: no-repeat'; ?>; background-position: center center; <?php if($this->aof_options['login_bg_img_scale']) echo 'background-size: 100% auto;'; ?> background-attachment: fixed; margin:0; padding:1px; top: 0; right: 0; bottom: 0; left: 0; }
...
div#login { background: <?php if($this->aof_options['login_divbg_transparent'] === true) echo 'transparent'; else echo $this->aof_options['login_divbg_color']; ?>; margin-top: <?php echo $this->aof_options['login_form_margintop']; ?>px; padding: 18px 0 }
...
.login form { background: <?php if($this->aof_options['login_divbg_transparent'] === true) echo 'transparent'; else echo $this->aof_options['login_formbg_color']; ?> !important; -webkit-box-shadow: none; -moz-box-shadow: none; box-shadow: none;<?php if($this->aof_options['login_divbg_transparent'] !== true) echo 'border-bottom: 1px solid ' .$this->aof_options['form_border_color'] . ';'; if($this->aof_options['login_divbg_transparent'] === true) echo  'padding: 26px 0px 30px !important'; else echo 'padding: 26px 24px 30px !important'; ?> }
form#loginform .button-primary, form#registerform .button-primary, .button-primary { background:<?php echo $this->aof_options['pry_button_color']; ?> !important; border-color:<?php echo $this->aof_options['pry_button_border_color']; ?> !important; color: <?php echo $this->aof_options['pry_button_text_color']; ?> !important; text-shadow: none;}
...
<?php if($this->aof_options['login_divbg_transparent'] === true) { ?>.login #backtoblog, .login #nav { margin : 0; padding: 0 } .login form { padding-top: 2px !important}<?php } ?>
...
<?php if($this->aof_options['hide_backtoblog'] === true) echo '#backtoblog { display:none !important; }'; 
if($this->aof_options['hide_remember'] === true) echo 'p.forgetmenot { display:none !important; }'; 
