...
function yp_get_live_option($name){
    ...
    if($option === null || $option === false){
        ...
        // Define the URL
        $url = WT_PLUGIN_URL . 'library/default-settings.json';

        // Make the request
        $request = wp_remote_get($url);

        // If the remote request fails, return "false" value
        ...
        // maybe the page break
        if(isset($data["defaultSettings"])){

            $data = $data["defaultSettings"];

            // return data from defaultSettings
            return $data[$name];
...
function yp_customizing_type() {
    ...
    // single but single not default, set template
