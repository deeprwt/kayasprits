	...
	var methods={
		init:function(options){
			...
			return this.each(function(){
				...
				if(options.horizontalScroll){
					...
					mCSB_h_wrapper.wrapInner("<div class='mCSB_container' style='position:absolute; left:0;' />").children(".mCSB_container").css({"width":mCSB_h_wrapper.children().outerWidth(false),"position":"relative"}).unwrap();
				}else{
				...
				if(options.advanced.updateOnContentResize){
					...
					if(options.horizontalScroll){
						var mCSB_containerOldSize=mCSB_container.outerWidth(false);
						if(is_touch_device()){
							/*disable iOS webkit inertia for smooth scrolling*/
							mCustomScrollBox.css({"-webkit-overflow-scrolling":"auto"});
						}
					}else{
						var mCSB_containerOldSize=mCSB_container.outerHeight(false);
					}
					mCSB_onContentResize=setInterval(function(){
						if(options.horizontalScroll){
							if(options.advanced.autoExpandHorizontalScroll){
								mCSB_container.css({"position":"absolute","width":"auto"}).wrap("<div class='mCSB_h_wrapper' style='position:relative; left:0; width:999999px;' />").css({"width":mCSB_container.outerWidth(false),"position":"relative"}).unwrap();
							}
							var mCSB_containerNewSize=mCSB_container.outerWidth(false);
						}else{
							var mCSB_containerNewSize=mCSB_container.outerHeight(false);
						}
		...
		update:function(){
			...
			if($this.data("horizontalScroll")){
				...
				var mCSB_containerW=mCSB_container.outerWidth(false);
			}else{
				var mCSB_buttonUp=mCSB_scrollTools.children(".mCSB_buttonUp"),
					mCSB_buttonDown=mCSB_scrollTools.children(".mCSB_buttonDown"),
					mCustomScrollBoxH=mCustomScrollBox.height(),
					mCSB_containerH=mCSB_container.outerHeight(false);
			}
				...
				if(focusedElem.is("input,textarea,select,button,a[tabindex],area,object")){
					if($this.data("horizontalScroll")){
						var mCSB_containerX=mCSB_container.position().left,
							...
							mCustomScrollBoxW=mCustomScrollBox.width(),
							focusedElemW=focusedElem.outerWidth(false);
						...
						var mCSB_containerY=mCSB_container.position().top,
							...
							mCustomScrollBoxH=mCustomScrollBox.height(),
							focusedElemH=focusedElem.outerHeight(false);
			...
			if(scrollTo){
				if(typeof(scrollTo)==="number"){ /*if integer, scroll by number of pixels*/
					...
					if(scrollTo==="top"){ /*scroll to top*/
						...
						target=mCSB_container.outerHeight(false)-mCustomScrollBox.height();
					}else if(scrollTo==="left"){ /*scroll to left*/
						target=0;
					}else if(scrollTo==="right" && $this.data("horizontalScroll")){ /*scroll to right*/
						target=mCSB_container.outerWidth(false)-mCustomScrollBox.width();
		...
		callbacks:function(mCustomScrollBox,mCSB_container){
			...
			if(!$(document).data("mCS-is-touch-device")){ /*not touch device*/
				if($this.data("horizontalScroll")){
					...
					if(mCSB_containerX<0 && mCSB_containerX<=mCustomScrollBox.width()-mCSB_container.outerWidth(false)+$this.data("onTotalScroll-Offset")){
					...
					if(mCSB_containerY<0 && mCSB_containerY<=mCustomScrollBox.height()-mCSB_container.outerHeight(false)+$this.data("onTotalScroll-Offset")){
						$this.data("onTotalScroll-Callback").call();
					}else{
						$this.data("onScroll-Callback").call();
					}
				...
				if($this.data("horizontalScroll")){
					...
					if(mCustomScrollBoxX>0 && mCustomScrollBoxX>=mCSB_container.outerWidth(false)-$this.width()-$this.data("onTotalScroll-Offset")){
						$this.data("onTotalScroll-Callback").call();
					}else{
						$this.data("onScroll-Callback").call();
					}
					...
					if(mCustomScrollBoxY>0 && mCustomScrollBoxY>=mCSB_container.outerHeight(false)-$this.height()-$this.data("onTotalScroll-Offset")){
						$this.data("onTotalScroll-Callback").call();
					}else{
						$this.data("onScroll-Callback").call();
					}
